...
public class MyMain {
	...
	public static void main( String[] args ) throws Exception{
		...
		conf.set("mapred.jar","D:\\boxlog\\myinputformat.jar"); 
		
		Path out= new Path( hdfs+"/maxvalueinput");
		out.getFileSystem(conf).delete(out,true);
		
		Job job = Job.getInstance(conf , "maxvalueinput") ;
	...
	public static class MyMapper extends Mapper<IntWritable, ArrayWritable, IntWritable, IntWritable> {
		public void map(IntWritable key, ArrayWritable value, Context context)
			...
			//写成1，表示两个split的最大值会放到一个reduce执行
